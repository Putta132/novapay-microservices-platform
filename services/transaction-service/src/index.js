...
const { startConsumer, stopConsumer } = require('./kafka/transactionConsumer');
...
app.get('/health', (_req, res) => res.json({ status: 'healthy', service: 'transaction-service', timestamp: new Date().toISOString() }));
...
const server = app.listen(PORT, () => logger.info(`transaction-service listening on port ${PORT}`));

// Start Kafka consumer — listens on novapay.payments topic to record transactions
// asynchronously when payment-service publishes payment.completed/failed events.
...
const shutdown = async () => {
  logger.info({ message: 'transaction-service shutting down, disconnecting Kafka consumer' });
  await stopConsumer();
